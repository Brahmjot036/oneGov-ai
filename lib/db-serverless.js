    ...
    prepare: (sql) => {
      return {
        get: (...params) => {
          ...
          if (sql.includes('SELECT') && sql.includes('FROM users')) {
            if (sql.includes('WHERE email = ?')) {
              ...
              const emailToFind = String(params[0]).toLowerCase().trim();
          ...
          if (sql.includes('SELECT') && sql.includes('FROM sessions')) {
            if (sql.includes('WHERE token = ?')) {
              ...
              if (session && new Date(session.expires_at) > new Date()) {
                ...
                return session ? { ...session, user_id: session.user_id, name: user?.name || null, email: user?.email || null } : null;
              }
              return null;
            }
          }
        ...
        all: (...params) => {
          ...
          // Debug: SELECT * FROM users (for debugging)
          if (sql.includes('SELECT') && sql.includes('FROM users') && !sql.includes('WHERE')) {
            return memoryDb.users.map(u => ({ id: u.id, email: u.email }));
          }
