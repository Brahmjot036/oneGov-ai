...
function getDb() {
  ...
  // This will help identify if the schema hasn't been set up
  console.log('🔍 Supabase client initialized, checking database connection...')
            ...
            if (sql.includes('WHERE email = ?')) {
              ...
              if (error) {
                ...
                if (error.code === '42P01' || error.message?.includes('does not exist')) {
                  // Table doesn't exist
                  console.error('❌ CRITICAL: Table "users" does not exist in Supabase!');
                  console.error('❌ Please run the SQL schema from supabase-setup.sql in your Supabase SQL Editor');
