  ...
  if (process.env.VERCEL || process.env.NEXT_PUBLIC_VERCEL_ENV) {
    ...
    try {
      if (isSupabaseConfigured()) {
        console.log('✅ Using Supabase database (persistent)');
        if (!supabaseDb) {
    ...
    console.warn('⚠️  Using in-memory database - data resets on each request!');
    console.warn('⚠️  To fix: Set up Supabase and add environment variables in Vercel');
    if (!serverlessDb) {
